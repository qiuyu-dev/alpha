...
//		batchFeeMst.setCashier(operator);
	...
	public Result listBatchFeeMstbyUser() throws CustomException {
		String operator = SecurityUtils.getSubject().getPrincipal().toString();
		List<BatchFeeMst> batchFeeMstList = batchFeeService.findAllBatchFeeMstByUserAndStateIn(operator,
				Arrays.asList(CustomStatus.STATUS6.value()));
...
//			}
			List<BatchFeeDetail> batchFeeDetailList = batchFeeService
					.findDetailByCustomerSubjectId(cpExcelDetail.getCustomerSubjectId());
			for (BatchFeeDetail batchFeeDetail : batchFeeDetailList) {
...
//				if (customerProduct.getState() < CustomStatus.STATUS5.value()) {
//					throw new CustomException(0, "请确认服务状态");
//				} else {
//					if (customerProduct.getState() == CustomStatus.STATUS5.value()) {
...
//			customerProductService.saveAll(customerProductList);
	...
	private void saveCustomerProduct( CpExcelDetail cpExcelDetail,Date effectiveDate ,Date closingDate ) throws CustomException {
			...
			for (Product product : productList) {
				if (!(customerProductService.isExistOutTradeNoe(cpExcelDetail.getCustomerSubjectId(), product.getId(),
						effectiveDate,
						closingDate))) {
