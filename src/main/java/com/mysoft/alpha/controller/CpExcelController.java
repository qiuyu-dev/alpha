		...
		for (CpExcelMst cpExcelMst : cpExcelMstList) {
			...
			case 1:
				cpExcelDetailList = cpExcelService
						.findDetailByParamsOrderByIdAsc(
								cpExcelMst.getId(), Arrays.asList(CustomStatus.STATUS3.value(),
										CustomStatus.STATUS4.value(), CustomStatus.STATUS_5.value()),
								name, recordNumber, productName, outTradeNo);
				break;
			}
...
//        System.out.println("----------------------saveCustomerProduct");
	...
	private CpExcelDetail parseOneRow(CpExcelMst cpExcelMst, Sheet sheet, int irows) throws Exception {
		...
		if (alphaSubjectService.isExistCustomerSubject(customerTypeName, recordNumber, customerName, sex,
			...
			customer = alphaSubjectService.findByParams(customerTypeName, recordNumber, customerName, sex,
					cpExcelDetail.getCustomerPhone());
		} else {
