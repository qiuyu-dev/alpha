...
//                .println("----------------------CpExcelController:detailList,cpExcelMstList:" + cpExcelMstList.size());
    ...
    public Result deleteCpExcelDetail(@RequestParam() Integer detailId) throws CustomException {
        ...
        CustomException {
                ...
                String chargeId = map.get("chargeid");//收费企业
            ...
            try {
                System.out.println("----------------------不存在:" + oriFileName);
                ...
                System.out.println("----------------------Exception:" + mstId);
                alphaSubjectService.deleteBySourceTypeAndSourceId(1, mstId);
                productService.deleteBySourceTypeAndSourceId(1, mstId);
                cpExcelService.deleteDetailByCpExcelMstId(mstId);
                cpExcelService.deleteMstById(mstId);
        ...
        private CpExcelDetail parseOneRow (CpExcelMst cpExcelMst, Sheet sheet,int irows) throws Exception {
            System.out.println("-----------------------parseOneRow=mstid:" + cpExcelMst.getId());// 行数包括标题行
            ...
            for (int icell = 0; icell <= 12; icell++) {
                System.out.println("icell:" + icell);// 行数包括标题行
                Cell cell = row.getCell(icell);
                String str = null;
               System.out.println("icell:"+icell+"dataformat"+ cell.getCellStyle().getDataFormatString());
            ...
            System.out.println("productService:");// 行数包括标题行
            //先存一次产生detailid
            ...
            if (alphaSubjectService.isExistAlphaSubject(1, recordType, recordNumber)) {
                customer =
                        alphaSubjectService.findBySubjectTypeAndRecordTypeAndRecordNumber(1, recordType, recordNumber);
            } else {
            ...
            System.out.println("----------------other:");// 行数包括标题行
            ...
            //        cpExcelDetail = cpExcelService.saveDetail(cpExcelDetail);
            System.out.println("saveDetail:");// 行数包括标题行
        ...
        public String getProvince (String id){
            ...
            String[] b =
                    {"北京市", "天津市", "河北省", "山西省", "内蒙古自治区", "辽宁省", "吉林省", "黑龙江省", "上海市", " 江苏省", "浙江省", "安徽省", "福建省",
                            ...
                            " 西藏自治区", "陕西省", "甘肃省", "青海省", "宁夏回族自治区", "新疆维吾尔自治区", "台湾省", "香港特别行政区",
                            "澳门特别行政区"};       //将省份全部放进数组b;
