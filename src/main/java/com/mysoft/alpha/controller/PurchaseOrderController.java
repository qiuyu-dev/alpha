...
public class PurchaseOrderController {
    ...
    public Result updateCustomerEnterprise(@RequestBody Map<String, String> map) {
        ...
        if ("1".equals(opt)) {// 1 通过
            customerEnterprise.setStatus("5");// 已核实
    ...
    public Result listBatchFeeMstbyUser() {
        String operator = SecurityUtils.getSubject().getPrincipal().toString();
        return ResultFactory.buildSuccessResult(batchFeeMstService.findAllBatchFeeMstByUser(operator));
    }
    ...
    public Result payConfirm(@RequestBody Map<String, String> map) {
        ...
        batchFeeMst.setCreateTime(new Date());
        batchFeeMst.setStatus("7");// 7 = "确认收款服务中";//服务方-客户或采购方
        batchFeeMst.setOperator(operator);
        ...
        for (BatchFeeDetail batchFeeDetail : batchFeeDetailList) {
            for (Product product : productList) {
                ...
                customerEnterprise = customerEnterpriseService.findCustomerEnterpriseById(batchFeeDetail.getCeId());
                customerProduct.setName(customerEnterprise.getCname());
                customerProduct.setCertificateType(customerEnterprise.getCertificateType());
                customerProduct.setInsuredId(customerEnterprise.getInsuredId());
                customerProduct.setPhone(customerEnterprise.getPhone());
    ...
    public Result listCustomerEnterpriseTo() {
        String operator = SecurityUtils.getSubject().getPrincipal().toString();
        List<String> statusList = Stream.of("3", "4").collect(Collectors.toList());
    ...
    public Result listCustomerEnterpriseFrom() {
        ...
        List<String> statusList = Stream.of("5", "6", "7", "8", "9", "-7").collect(Collectors.toList());
        List<CustomerEnterprise> ceList =
